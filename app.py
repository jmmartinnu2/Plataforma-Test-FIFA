...
import os
import time
from streamlit_autorefresh import st_autorefresh  # Asegúrate de instalar esta librería
...
for key in [
    'sesion_iniciada', 'modo_prueba', 'historial_preguntas',
    'mostrar_resultados', 'ver_correccion', 'respuestas_correctas',
    'resultados', 'feedback', 'exam_manager', 'end_time'
]:
    if key not in st.session_state:
        st.session_state[key] = False if key in ['sesion_iniciada', 'modo_prueba', 'mostrar_resultados',
                                                'ver_correccion', 'exam_manager'] else []
...
# **9. Función para Mostrar el Temporizador**
def mostrar_temporizador():
    if 'end_time' in st.session_state and isinstance(st.session_state['end_time'], datetime):
        now = datetime.now()
        remaining_time = st.session_state['end_time'] - now

        if remaining_time.total_seconds() > 0:
            mins, secs = divmod(int(remaining_time.total_seconds()), 60)
            time_str = f"⏳ Tiempo restante: {mins:02}:{secs:02}"
            if mins < 5:
                st.sidebar.warning("⚠️ Quedan menos de 5 minutos.")
            else:
                st.sidebar.info(time_str)
            return True
        else:
...
def configurar_examen_prueba():
    ...
    st.session_state['end_time'] = st.session_state['start_time'] + timedelta(minutes=exam_manager.get_tiempo_limite())
...
def iniciar_examen_normal(num_preguntas, tiempo_limite, orden_aleatorio, temas_seleccionados):
    ...
    st.session_state['mostrar_modal'] = True  # Mostrar la modal
    rerun_app()  # Actualiza la interfaz

# **13. Función para Renderizar la Modal (Opcional)**
def render_modal():
    tiempo_restante = mostrar_temporizador()
    if tiempo_restante:
        st.markdown("""
            <style>
                .modal {
        ...
        """.format(f"⏳ Tiempo restante: {mins:02}:{secs:02}"), unsafe_allow_html=True)

# **14. Lógica Principal de la Aplicación**
def main():
    # **Autorefresh cada segundo si el examen está activo**
    if 'exam_manager' in st.session_state and st.session_state['end_time']:
        now = datetime.now()
        remaining_time = st.session_state['end_time'] - now
        if remaining_time.total_seconds() > 0:
    ...
    if not st.session_state['sesion_iniciada'] and not st.session_state['modo_prueba'] and not st.session_state['exam_manager']:
        ...
        # **15. Añadir Banner de Contacto por WhatsApp**
        st.sidebar.markdown(
            """
            ---
